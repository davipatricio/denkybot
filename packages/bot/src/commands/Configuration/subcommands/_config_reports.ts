...
type PageTypes = 'report' | 'anonymous' | 'channel';

export default class ReportSubCommand extends SubCommand {
  async run({ t, interaction }: CommandRunOptions) {
    ...
    const { embed, buttonRow } = this.updateMessage('report', null, selectRow, interaction, configStatus, t);
    ...
    const paginationSelect = new SelectMenuBuilder()
      ...
      .setPlaceholder('Páginas')
      .setOptions([
        new SelectMenuOptionBuilder().setDescription('Ative ou desativa o sistema de denúncias').setLabel('Denúncias').setValue('report').setEmoji('🚫'),
        new SelectMenuOptionBuilder().setDescription('Defina o canal que será usado para as denúncias').setLabel('Canal').setValue('channel').setEmoji('📰'),
        new SelectMenuOptionBuilder().setDescription('Ativa ou desativa as denúncias anônima').setLabel('Denúncias anônimas').setValue('anonymous').setEmoji('🕵️')
      ]);
